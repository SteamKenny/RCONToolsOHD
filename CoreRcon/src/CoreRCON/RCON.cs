...
namespace CoreRCON
{
    ...
    /// <param name="dynamicMultiPacketSupport">Enable dynamic multi packet response handling for servers that do not support 'sourceMultiPacketSupport'</param>
    ...
    public partial class RCON(
        ...
        bool dynamicMultiPacketSupport = false,
        ...
        ILogger logger = null) : IDisposable
    {
        ...
        private readonly bool _sourceMultiPacketSupport = sourceMultiPacketSupport;
        private readonly bool _dynamicMultiPacketSupport = dynamicMultiPacketSupport;
        ...
        private Dictionary<int, System.Timers.Timer> _incomingTimers { get; } = [];
        ...
        /// <param name="dynamicMultiPacketSupport"></param>
        ...
        public RCON(IPAddress host,
            ...
            bool autoConnect = true,
            ILogger logger = null)
            : this(new IPEndPoint(host, port), password, timeout, sourceMultiPacketSupport, dynamicMultiPacketSupport, strictCommandPacketIdMatching, autoConnect, logger)
        { }
        ...
        private void RCONPacketReceived(RCONPacket packet)
        {
            ...
            // 4082 characters is the longest payload that can arrive in a packet, if we see this then 99% of the
            // time it will be followed by one or more further packets that constitute a multi-packet response.
            // For the 1% where the packet length just happpens to be 4082, the timer will elapse and conclude the
            // response.
            else if (_dynamicMultiPacketSupport && packet.Body.Length == 4082)
            {
                //Highly likely that this is a multi-packet response
                _logger?.LogTrace("RCON packet received: {} [DMPS - Full Packet] Length: {}", packet.Id, packet.Body.Length);
                ...
                if (_incomingTimers.ContainsKey(packetId))
                {
                    //Reset timer
                    _incomingTimers[packetId].Start();
                }
                else
                {
                    //Create timer (for the 1% chance of a 4082 single packet response and also to generally protect against infinite waiting for subsequent packets)
                    var timer = new System.Timers.Timer() { Interval = 500, AutoReset = false };
                    timer.Elapsed += (sender, e) =>
                    {
                        _logger?.LogTrace("DMPS Timer Elapsed, Response Closed for: {}", packetId);

                        //Assume no more data for this multi-packet response
                        _incomingTimers[packetId].Stop();
                        _incomingTimers[packetId].Dispose();
                        _incomingTimers.Remove(packetId);

                        //Avoid yielding
                        taskSource.SetResult(_incomingBuffer[packetId]);
                        _pendingCommands.TryRemove(packetId, out _);
                    };
                    _incomingTimers[packetId] = timer;
                    timer.Start();
                }
            }
            else
            {
                ...
                //Read any previous messages
                if (_incomingBuffer.TryGetValue(packetId, out string body))
                {
                    _logger?.LogTrace("RCON packet received: {} [DMPS - Final Packet] Length: {}", packet.Id, packet.Body.Length);
        ...
        private async Task SendPacketAsync(RCONPacket packet)
        {
            ...
            await _tcp.SendAsync(new ArraySegment<byte>(packet.ToBytes()), SocketFlags.None)
                .ConfigureAwait(false);
            if (packet.Type == PacketType.ExecCommand && _sourceMultiPacketSupport)
            {
